...
add_action('admin_menu', 'product_add_boxel');
// Add meta boxel
function product_add_boxel() {
	global $product_meta_boxel;
	foreach ( array( 'product' ) as $page )
	add_meta_box($product_meta_boxel['id'], $product_meta_boxel['title'], 'product_show_boxel', $page, $product_meta_boxel['context'], 			$product_meta_boxel['priority']);
}
// Callback function to show fields in meta boxel
function product_show_boxel() {
	global $product_meta_boxel, $post;
...
jQuery(document).ready(function() {
	...
	echo '<input type="hidden" name="product_product_meta_boxel_nonce" value="', wp_create_nonce(basename(__FILE__)), '" />';
	echo '<table class="form-table">';
	foreach ($product_meta_boxel['fields'] as $field) {
...
function product_save_data($post_id) {
	global $product_meta_boxel;	
	// verify nonce
	if (!wp_verify_nonce($_POST['product_product_meta_boxel_nonce'], basename(__FILE__))) {
		return $post_id;
	}
	// check autosave

...
class WC_Subscriptions_Order {
	...
	public static function pre_process_shop_order_meta( $post_id, $post ) {
		...
		if ( ! isset( $_POST['_order_recurring_tax_total'] ) ) { // WC 2.1+
			update_post_meta( $post_id, '_order_recurring_tax_total', woocommerce_round_tax_total( $total_tax ) );
		}

		if ( ! isset( $_POST['_order_recurring_shipping_tax_total'] ) ) { // WC 2.1+
			update_post_meta( $post_id, '_order_recurring_shipping_tax_total', woocommerce_round_tax_total( $total_shipping_tax ) );
		}
		...
		if ( isset( $_POST['recurring_shipping_method_id'] ) ) { // WC 2.1+
			...
			foreach( $recurring_shipping_method_id as $item_id => $value ) {
				...
				if ( $item_id == 'new' ) {
					...
					woocommerce_add_order_item_meta( $item_id, 'method_id', $method_id );
					woocommerce_add_order_item_meta( $item_id, 'cost', $cost );
...
<div id="recurring_order_totals"<?php if ( ! $contains_subscription ) echo $display_none; ?>>
	...
	<div class="totals_group">
		...
		<input type="text" class="wc_input_price"  id="_order_recurring_discount_total" name="_order_recurring_discount_total" placeholder="0.00" value="<?php echo self::get_recurring_discount_total( $order ); ?>" style="margin: 6px 0 10px;"/>
	</div>
	...
	<div class="totals_group">
		...
		<input type="text" id="_order_recurring_total" name="_order_recurring_total" placeholder="0.00" value="<?php echo self::get_recurring_total( $order ); ?>" class="calculated"  style="margin: 6px 0 10px;"/>
	</div>

	<div class="totals_group">
		<h4><?php _e( 'Recurring Payment Method:', 'woocommerce-subscriptions' ); ?></h4>
		<div style="padding-top: 4px; font-style: italic; margin: 2px 0 10px;"><?php echo ( $manual_renewal ) ? __( 'Manual', 'woocommerce-subscriptions' ) : $order->recurring_payment_method_title; ?></div>
	</div>
<?php
	}

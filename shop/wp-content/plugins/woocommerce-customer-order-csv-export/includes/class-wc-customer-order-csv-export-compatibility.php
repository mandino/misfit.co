...
class WC_Customer_Order_CSV_Export_Compatibility {
	...
	private function get_import_one_column_per_line_item( $order_data, WC_Order $order ) {
		...
		if ( SV_WC_Plugin_Compatibility::is_wc_version_gte_2_1() ) {
			...
			foreach ( $order->get_items( 'shipping' ) as $_ => $shipping_item ) {
				...
				$order_data[ "shipping_cost_{$count}" ]   = SV_WC_Plugin_Compatibility::wc_format_decimal( $shipping_item['cost'], 2 );
			...
			$order_data['shipping_method_1'] = $order->shipping_method;
			$order_data['shipping_cost_1'] = $order->get_shipping();
		}
	...
	private function get_legacy_one_row_per_line_item( $order_data, WC_Order $order ) {
		...
		foreach ( $order->get_items() as $_ => $item ) {
			...
			if ( isset( SV_WC_Plugin_Compatibility::WC()->countries->countries[ $order->billing_country ] ) ) {
				$order_data['billing_country'] = SV_WC_Plugin_Compatibility::WC()->countries->countries[ $order->billing_country ];
			}
	...
	private function get_max_shipping_line_items( $order_ids ) {
		...
		// WC versions prior to 2.1 only had a single shipping method per order
		if ( ! SV_WC_Plugin_Compatibility::is_wc_version_gte_2_1() ) {
			return 1;
		}

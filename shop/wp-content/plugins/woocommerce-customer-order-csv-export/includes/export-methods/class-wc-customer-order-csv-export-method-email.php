...
class WC_Customer_Order_CSV_Export_Method_Email implements WC_Customer_Order_CSV_Export_Method {
	...
	private function email_export( $filename ) {
		...
		// init email args
		$mailer  = SV_WC_Plugin_Compatibility::WC()->mailer();
		...
		$message = sprintf( __( 'Order Export for %s', WC_Customer_Order_CSV_Export::TEXT_DOMAIN ), date_i18n( SV_WC_Plugin_Compatibility::is_wc_version_gte_2_1() ? wc_date_format() : woocommerce_date_format(), current_time( 'timestamp' ) ) );

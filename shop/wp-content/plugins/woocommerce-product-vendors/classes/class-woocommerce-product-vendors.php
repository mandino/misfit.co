...
class WooCommerce_Product_Vendors {
    ...
    public function process_commissions( $order_id ) {
        ...
        // Include coupons if option is set
        ...
        foreach( $items as $item_id => $item ) {
            ...
            foreach( $discounts as $product => $data ) {
                ...
                if( ! in_array( $product, array( 'all', $product_id ) ) ) continue;
    ...
    public function product_vendor_tab( $tabs ) {
        ...
        if( $vendors ) {
            if( count( $vendors ) > 1 ) {
                ...
                $title = __( 'Vendor', 'wc_product_vendors' );
            }
            $tabs['vendor'] = array(
                'title' => $title,
                'priority' => 20,
    ...
    public function add_settings( $settings ) {
        ...
        $vendors_settings = array(
            array(  'title' => __( 'Product Vendors', 'wc_product_vendors' ), 'type' => 'title', 'id' => 'product_vendors_options' ),

            array(
                'title'         => __( 'Product listings', 'wc_product_vendors' ),
                'desc'          => __( 'Hide vendor products from other product listings', 'wc_product_vendors' ),
                'id'            => 'woocommerce_product_vendors_hide_vendor_products',
                'default'       => 'no',
                'desc_tip'      => __( 'Hide products belonging to vendors from other product listings - this means that vendor products will only be visible on the individual vendor pages.', 'wc_product_vendors' ),
                'type'          => 'checkbox',
                'checkboxgroup' => 'start'
            ),

            array(
                'title'         => __( 'Admin review', 'wc_product_vendors' ),
                'desc'          => __( 'Skip admin review for vendors', 'wc_product_vendors' ),
                'id'            => 'woocommerce_product_vendors_skip_review',
                'default'       => 'no',
                'desc_tip'      => __( 'Allow vendors to add products without the need for admin review - after changing this option you will need to resave each vendor.', 'wc_product_vendors' ),
                'type'          => 'checkbox',
                'checkboxgroup' => 'start'
            ),
            ...
            array(
                ...
                'desc'      => __( 'This sets the default commission percentage that will be applied if no product or vendor commission is specified.', 'wc_product_vendors' ),
            ...
            array(
                'title'         => __( 'Coupon handling', 'wc_product_vendors' ),
                'desc'          => __( 'Include coupons in commission calculations', 'wc_product_vendors' ),
                'id'            => 'woocommerce_product_vendors_include_coupons',
                'default'       => 'no',
                'desc_tip'      => __( 'Decide whether vendor commissions should take coupons into account or not.', 'wc_product_vendors' ),
                'type'          => 'checkbox',
                'checkboxgroup' => 'start'
            ),

            array( 'type' => 'sectionend', 'id' => 'product_vendors_options' ),
        );

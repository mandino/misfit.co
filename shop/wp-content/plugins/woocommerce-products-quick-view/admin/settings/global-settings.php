	...
	public function init_form_fields() {
     	...
     	$this->form_fields = apply_filters( $this->option_name . '_settings_fields', array(
			...
			array(  
				...
				'id' 		=> 'quick_view_ultimate_popup_content_pro',
				'type' 		=> 'onoff_radio',
			...
			array(  
				'name' 		=> __( "Simple Custom Template Pop-up", 'wooquickview' ),
				...
				'onoff_options' => array(
					array(
						'val' 				=> 'custom_template',
						'text' 				=> __( 'Use the Custom Template for simple pop-up with Dynamic product image gallery', 'wooquickview' ).' <span class="description">('.__( 'recommended', 'wooquickview' ).')</span>' ,
						'checked_label'		=> 'ON',
						'unchecked_label' 	=> 'OFF',
					),
				),
...
$(document).ready(function() {
	if ( $("input[name='quick_view_ultimate_enable']:checked").val() != 'yes') {
		...
		$('.pro_feature_fields').css( {'visibility': 'hidden', 'height' : '0px', 'overflow' : 'hidden', 'margin-bottom' : '0px' } );
	}
	...
	$(document).on( "a3rev-ui-onoff_checkbox-switch", '.quick_view_ultimate_enable', function( event, value, status ) {
		...
		$('.pro_feature_fields').attr('style','display:none;');
		if ( status == 'true' ) {
			$(".quick_view_ultimate_container").slideDown();
			$(".pro_feature_fields").slideDown();
		} else {
			$(".quick_view_ultimate_container").slideUp();
			$(".pro_feature_fields").slideUp();
		}
	});
});
})(jQuery);

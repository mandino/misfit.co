...
class WC_Name_Your_Price_Admin {
	...
	public static function save_variable_product_meta( $post_id ){
		...
		if ( isset( $_POST['variable_sku'] ) ) :
			...
			for ( $i = 0; $i < sizeof( $variable_sku ); $i++ ) :
				$nyp = isset( $variation_is_nyp[ $i ] ) ? 'yes' : 'no';
    ...
    public static function meta_box_script( $hook ){
		...
		wp_enqueue_script( 'woocommerce_nyp_metabox',WC_Name_Your_Price()->plugin_url() . '/includes/admin/js/nyp-metabox.js', array( 'jquery' ), WC_Name_Your_Price()->version, true );
	...
	public static function quick_edit_scripts( $hook ) {
		...
		if ( $hook == 'edit.php' && $post_type == 'product' )
			wp_enqueue_script( 'nyp-quick-edit', WC_Name_Your_Price()->plugin_url() . '/includes/admin/js/nyp-quick-edit.js', array( 'jquery' ), WC_Name_Your_Price()->version, true );

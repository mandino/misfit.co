...
class WC_Name_Your_Price_Display {
	...
	function nyp_scripts() {
		...
		wp_enqueue_script( 'accounting', WC_Name_Your_Price()->plugin_url() . '/assets/js/accounting.js', '', '0.3.2', true );
		wp_enqueue_script( 'woocommerce-nyp', WC_Name_Your_Price()->plugin_url() . '/assets/js/name-your-price.js', array( 'jquery', 'accounting', 'wc-add-to-cart-bundle' ), WC_Name_Your_Price()->version, true );
	...
	public function display_price_input( $product_id = false, $prefix = false ){
		...
		// load up the NYP scripts ( before the bundles script )
		add_action( 'wp_print_footer_scripts', array( $this, 'nyp_scripts' ), 5 );

...
/**
 * WC_Gateway_Stripe_Connect_Subscriptions class.
 *
 * @extends WC_Gateway_Stripe_Connect
 */
class WC_Gateway_Stripe_Connect_Subscriptions extends WC_Gateway_Stripe_Connect {
	...
	function process_subscription_payment( $order = '', $amount = 0 ) {
		...
		$subscription_name = sprintf( __( 'Subscription for "%s"', 'wc_stripe_connect' ), $product->get_title() ) . ' ' . sprintf( __( '(Order %s)', 'wp_stripe' ), $order->get_order_number() );

		if ( $amount * 100 < 50 )
			return new WP_Error( 'stripe_error', __( 'Minimum amount is 0.50', 'wc_stripe_connect' ) );

		$stripe_customer = get_post_meta( $order->id, '_stripe_customer_id', true );

		if ( ! $stripe_customer )
			return new WP_Error( 'stripe_error', __( 'Customer not found', 'wc_stripe_connect' ) );
		...
		if ( is_wp_error( $response ) ) {
			...
			$order->add_order_note( sprintf( __('Stripe subscription payment completed (Charge ID: %s)', 'wc_stripe_connect' ), $response->id ) );
	...
	function add_customer_to_order( $order, $customer_id = '', $stripe_token = '' ) {
		...
		elseif ( $stripe_token ) {
			...
			if ( is_wp_error( $response ) ) {
				...
				$order->add_order_note( sprintf( __('Stripe customer added: ', 'wc_stripe_connect' ), $response->id ) );

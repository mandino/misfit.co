...
jQuery( function() {
	...
	/* Checkout Form */
	jQuery('form.checkout').on('checkout_place_order_stripe-connect', function( event ) {
...
function stripeFormHandler() {
	if ( jQuery('#payment_method_stripe-connect').is(':checked') && ( jQuery('input[name=stripe_customer_id]:checked').size() == 0 || jQuery('input[name=stripe_customer_id]:checked').val() == 'new' ) ) {

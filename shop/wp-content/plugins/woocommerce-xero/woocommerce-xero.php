...
*/
/*  Copyright 2012  Daniel Espinoza  (email : daniel@growdevelopment.com)
...
if ( is_woocommerce_active() ) {
	...
	function wc_xero_plugin_links( $links ) {
		...
		$plugin_links = array(
			'<a href="' . admin_url( 'admin.php?page=woocommerce_xero' ) . '">' . __( 'Settings', 'wc_xero' ) . '</a>',
			'<a href="http://www.woothemes.com/support/">' . __( 'Support', 'wc_xero' ) . '</a>',
			'<a href="http://docs.woothemes.com/document/xero/">' . __( 'Documentation', 'wc_xero' ) . '</a>',
		);
	...
	class WC_Xero {
	    ...
	    */
		public function __construct() {
			...
			if ( version_compare( WOOCOMMERCE_VERSION, '2.0.0', '<' ) ) {
				add_action('woocommerce_process_shop_order_meta', array( $this, 'xero_process_order_meta'), 20, 2 );
			} else {
			...
			if ( $this->send_invoices == 'on' ) {
				add_action('woocommerce_checkout_order_processed',array(&$this,'xero_new_invoice'));
			}

			if ( $this->send_payments == 'on' ) {
				add_action('woocommerce_payment_complete',array(&$this,'xero_new_payment'));
			}
		...
		*/
		function menu() {
			add_submenu_page('woocommerce', __('Xero', 'wc_xero'),  __('Xero', 'wc_xero') , 'manage_woocommerce', 'woocommerce_xero', array(&$this,'options_page'));
		}
				...
				<form method="post" id="mainform" action="options.php">
					...
					<?php
						if (isset($_GET['settings-updated']) && ($_GET['settings-updated'] == 'true')){
							 echo '<div id="message" class="updated fade"><p><strong>' . __( 'Your settings have been saved.', 'wc_xero' ) . '</strong></p></div>';

						} else if (isset($_GET['settings-updated']) && ($_GET['settings-updated'] == 'false')){
							 echo '<div id="message" class="error fade"><p><strong>' . __( 'There was an error saving your settings.', 'wc_xero' ) . '</strong></p></div>';
						}
					?>
		...
		*/
		function settings_init() {
			...
			$settings = array(
				array(
					...
					'title' 	=> __('Xero Settings','wc_xero'),
					'page'		=> 'woocommerce_xero',
					'settings'	=> array(
						array(
							'name'		=> 'wc_xero_consumer_key',
							'title'		=> __('Consumer Key','wc_xero'),
						),
						array(
							'name'		=> 'wc_xero_consumer_secret',
							'title'		=> __('Consumer Secret','wc_xero'),
						),
						array(
							'name'		=> 'wc_xero_private_key',
							'title'		=> __('Private Key','wc_xero'),
						),
						array(
							'name'		=> 'wc_xero_public_key',
							'title'		=> __('Public Key','wc_xero'),
						),
						array(
							'name'		=> 'wc_xero_sales_account',
							'title'		=> __('Sales Account','wc_xero'),
						),
						array(
							'name'		=> 'wc_xero_discount_account',
							'title'		=> __('Discount Account','wc_xero'),
						),
						array(
							'name'		=> 'wc_xero_shipping_account',
							'title'		=> __('Shipping Account','wc_xero'),
						),
						array(
							'name'		=> 'wc_xero_payment_account',
							'title'		=> __('Payment Account','wc_xero'),
						),
						array(
							'name'		=> 'wc_xero_send_invoices',
							'title'		=> __('Auto Send Invoices','wc_xero'),
						),
						array(
							'name'		=> 'wc_xero_send_payments',
							'title'		=> __('Auto Send Payments','wc_xero'),
						),
						array(
							'name'		=> 'wc_xero_send_inventory',
							'title'		=> __('Send Inventory Items','wc_xero'),
						),
						array(
							'name'		=> 'wc_xero_export_zero_amount',
							'title'		=> __('Orders with zero total','wc_xero'),
						),
						array(
							'name'		=> 'wc_xero_debug',
							'title'		=> __('Debug','wc_xero'),
						),
					),
				),
			);
		 ...
		 */
		function wc_xero_settings(){ echo '<p>'. __( 'Settings for your Xero account including security keys and default account numbers.<br/> <strong>All</strong> text fields are required for the integration to work properly.', 'wc_xero'). '</p>'; }
		function wc_xero_consumer_key(){ global $woocommerce; echo '<input type="text"  style="width: 300px;" name="wc_xero_consumer_key" id="wc_xero_consumer_key" value="'.get_option('wc_xero_consumer_key').'" />';
		...
		function wc_xero_private_key(){  
			...
			if ( file_exists( get_option('wc_xero_private_key') ) ) {
				echo '<p><span style="padding: .5em; background-color: #4AB915; color: #fff; font-weight: bold;">' . __('Key file found.', 'wc_xero') . '</span></p>';			
			} else {
				echo '<p><span style="padding: .5em; background-color: #bc0b0b; color: #fff; font-weight: bold;">' . __('Key file not found.','wc_xero') . '</span></p>';
				$working_dir = str_replace( 'wp-admin', '', getcwd()) ; 
				echo  '<p>' . __('  This setting should include the absolute path to the file which might include working directory: ','wc_xero') . '<span class="code" style="background: #efefef;">'. $working_dir . '</span></p>';
			}
		...
		function wc_xero_public_key(){  
			...
			if ( file_exists( get_option('wc_xero_public_key') ) ) {
				echo '<p><span style="padding: .5em; background-color: #4AB915; color: #fff; font-weight: bold;">' . __('Key file found.', 'wc_xero') . '</span></p>';			
			} else {
				echo '<p><span style="padding: .5em; background-color: #bc0b0b; color: #fff; font-weight: bold;">' . __('Key file not found.','wc_xero') . '</span></p>';
				$working_dir = str_replace( 'wp-admin', '', getcwd()) ; 
		...
		function wc_xero_sales_account(){ echo '<input type="text" name="wc_xero_sales_account" id="wc_xero_sales_account" value="'.get_option('wc_xero_sales_account').'" />';
											echo '<p class="description">Code for Xero account to track sales.</p>'; }
		function wc_xero_discount_account(){ echo '<input type="text" name="wc_xero_discount_account" id="wc_xero_discount_account" value="'.get_option('wc_xero_discount_account').'" />';
											echo '<p class="description">Code for Xero account to track customer discounts.</p>'; }
		function wc_xero_shipping_account(){ echo '<input type="text" name="wc_xero_shipping_account" id="wc_xero_shipping_account" value="'.get_option('wc_xero_shipping_account').'" />';
											echo '<p class="description">Code for Xero account to track shipping charges.</p>'; }
		function wc_xero_payment_account(){ echo '<input type="text" name="wc_xero_payment_account" id="wc_xero_payment_account" value="'.get_option('wc_xero_payment_account').'" />';
 		...
 		function wc_xero_send_invoices(){ $checked = (get_option('wc_xero_send_invoices')=='on') ? 'checked="checked"' : '';
				 							   echo '<input type="checkbox" name="wc_xero_send_invoices" id="wc_xero_send_invoices"  '.$checked.' /> '.__('Send Invoices to Xero automatically when order is placed.','wc_xero');}
 		function wc_xero_send_payments(){ $checked = (get_option('wc_xero_send_payments')=='on') ? 'checked="checked"' : '';
				 							   echo '<input type="checkbox" name="wc_xero_send_payments" id="wc_xero_send_payments"  '.$checked.' /> '.__('Send Payments to Xero automatically when payment is complete.','wc_xero');
		...
		function wc_xero_send_inventory(){ $checked = (get_option('wc_xero_send_inventory')=='on') ? 'checked="checked"' : '';
				 							   echo '<input type="checkbox" name="wc_xero_send_inventory" id="wc_xero_send_inventory"  '.$checked.' /> '.__('Send Item Code field with invoices','wc_xero');
 		...
 		function wc_xero_export_zero_amount(){ $checked = (get_option('wc_xero_export_zero_amount')=='on') ? 'checked="checked"' : '';
				 							   echo '<input type="checkbox" name="wc_xero_export_zero_amount" id="wc_xero_export_zero_amount"  '.$checked.' /> '.__('Export orders with zero total','wc_xero'); }
		function wc_xero_debug(){ $checked = (get_option('wc_xero_debug')=='on') ? 'checked="checked"' : '';
				 				  echo '<input type="checkbox" name="wc_xero_debug" id="wc_xero_debug"  '.$checked.' /> ' . __('Enable logging.  Log file is located at: /wp-content/plugins/woocommerce/logs/xero.txt','wc_xero'); }
		/**
		...
		* Xero Invoice
		*
		* Creat a new invoice for the current purchase.  The customer is created, or if it already
		...
		*/
		function xero_new_invoice($order_id) {
			...
			$xml .= '<Date>' . date('Y-m-d') . 'T'. date('H:i:s'). '</Date>';
			$xml .= '<DueDate>' . date('Y-m-d') . 'T'. date('H:i:s'). '</DueDate>';		    
			...
			foreach( $items as $key => $value ) {
				...
				$xml .= '<LineItem>';
				$xml .= '<Description>' . str_replace($replace_pattern, '""', $value['name']) .'</Description>';
			...
			// Add Cart discount as Line Item			
/*			if ( $order->cart_discount > 0 ) {
				$xml .= '<LineItem>';
				$xml .= '<Description>Cart Discount</Description>';
				$xml .= '<Quantity>1</Quantity>';
				if ( strlen($this->discount_account) != '' ){
					$xml .= '<AccountCode>' . $this->discount_account . '</AccountCode>';
				}
				$xml .= '<UnitAmount>-' . $order->cart_discount . '</UnitAmount>';
				$xml .= '</LineItem>';
			}
*/			
			...
			if($oauth_problem){
				...
				$order->add_order_note( __('XERO: Invoice not created. OAuth Error: ', 'wc_xero') . ' ' . $oauth_problem . ' | ' . $oauth_problem_advice );
				return;
			}
			...
			if ( $response->Status == 'OK' )  {
				...
				$order->add_order_note( __('Xero Invoice created.  ', 'wc_xero') .
				...
				$error_message = $response->Elements->DataContractBase->ValidationErrors->ValidationError->Message ? $response->Elements->DataContractBase->ValidationErrors->ValidationError->Message : __('None', 'wc_xero'); 
				
				$order->add_order_note( __('ERROR creating Xero invoice: ', 'wc_xero') .
				__(' ErrorNumber: ', 'wc_xero') . $response->ErrorNumber .
				__(' ErrorType: ', 'wc_xero') . $response->Type .
				__(' Message: ', 'wc_xero') . $response->Message .
				__(' Detail: ', 'wc_xero') . $error_message );
				$this->add_log('END XERO NEW INVOICE' );
			}
		...
		*/
		function xero_new_payment($order_id) {
			...
			if ( strlen( $invoice_id ) == 0 ) {
				$order->add_order_note( __('XERO Payment Error adding payment for order. A valid invoice number was not recorded.', 'wc_xero'));
				$this->add_log( __('Error adding payment for order ID:','wc_xero') . $order_id . __('.  A valid invoice number was not recorded.','wc_xero'));
				return;
			}

			// check for valid Currency Rate
			if ( strlen( $currency_rate ) == 0 ) {
				$order->add_order_note( __('XERO Payment Error adding payment for order. A valid currency rate was not recorded.', 'wc_xero'));
				$this->add_log( __('Error adding payment for order ID:','wc_xero') . $order_id . __('.  A valid currency rate was not recorded.','wc_xero'));
				return;
			}

			// Need a Payment Account number
			if ( strlen( $this->payment_account ) == '' ) {
				$order->add_order_note( __('XERO Payment Error adding payment for order ID. Payment Account number is blank.', 'wc_xero'));
				$this->add_log( __('Payment Error adding payment for order ID:','wc_xero') . $order_id . __('.  Payment Account number is blank.','wc_xero'));
				return;
			}
			...
			if( isset( $returned_items['oauth_problem'] ) ){
				...
				$order->add_order_note( __('XERO: Payment not created. OAuth Error: ', 'wc_xero') . ' ' . $returned_items['oauth_problem']  );
				return;
			}
			...
			if ( $response->Status == 'OK' )  {
				...
				$this->add_log('XERO RESPONSE:' . "\n" . $r );
				$order->add_order_note( __('Xero Payment created.  ', 'wc_xero') .
				...
				$order->add_order_note( __('ERROR creating Xero payment. ErrorNumber:' . $error_num . '| Error Message:' . $error_msg , 'wc_xero'));
				$this->add_log('END XERO NEW PAYMENT' );
			}
		...
		function xero_order_actions( $actions ) {
			...
			if ( version_compare( WOOCOMMERCE_VERSION, '2.0.0', '<' ) ) {
				?>
			    <li class="wide"><input type="submit" class="button tips" name="xero_invoice" value="<?php _e('Send Invoice to Xero', 'wc_xero'); ?>" data-tip="<?php _e('Send this order to Xero. Check Order Notes for success/failure.', 'wc_xero'); ?>" /></li>
		        <li class="wide"><input type="submit" class="button tips" name="xero_payment" value="<?php _e('Send Payment to Xero', 'wc_xero'); ?>" data-tip="<?php _e('Send this payment to Xero. Check Order Notes for success/failure.', 'wc_xero'); ?>" /></li>
				<?php
			} else {		
				if ( is_array( $actions ) ) {
					$actions['xero_manual_invoice'] = __('Send Invoice to Xero', 'wc_xero');
					$actions['xero_manual_payment'] = __('Send Payment to Xero', 'wc_xero');
				}
			}
		...
		/**
		 * Hanle the order actions buttons callback
		...
		function add_log( $message ) {
			...
			$this->log->add( $file='xero', $message );
		}

...
if ( ! function_exists( 'woothemes_add_javascript' ) ) {
	function woothemes_add_javascript() {  
		...
		if ( $load_slider_js ) { wp_enqueue_script( /*'widgetSlider'*/ 'flexslider' ); }
		...
		// Send localized strings to the "general" JavaScript.
		$data = array( 'select_a_page' => __( 'Select a page:', 'woothemes' ) );
...
if ( ! function_exists( 'woothemes_add_css' ) ) {
	function woothemes_add_css() {
		...
		if ( isset( $woo_options['woo_layout_width'] ) ) {
			...
			wp_register_style( 'woo-responsive-layout', esc_url( $dir ) . '/css/generator.php?layout_width=' . intval( $layout_width ), array(), '1.0.0', 'screen' );

			wp_enqueue_style( 'woo-responsive-layout' );
		}

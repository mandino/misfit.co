	...
	<table class="bar_chart">
		<thead>
			<tr>
				...
				<th><?php _e('Carts Abandoned', 'woocommerce'); ?></th>
			</tr>
		</thead>
		<tbody>
			<?php
				...
				foreach ($found_products as $product_id => $sales) :
					...
					if ($product) :
						...
						$orders_link = admin_url('edit.php?s&post_status=all&post_type=carts&action=-1&s=' . urlencode($product->post_title) . '&start_date=' . date("Y-m-d",$start_date) . '&end_date=' . date("Y-m-d",$end_date) . '&mv=Abandoned');
					else :
						$product_name = __('Product does not exist', 'woocommerce');
						$orders_link = admin_url('edit.php?s&post_status=all&post_type=carts&action=-1&start_date=' . date("Y-m-d",$start_date) . '&end_date=' . date("Y-m-d",$end_date) .'&mv=Abandoned');
					endif;
	...
	if ( $chosen_product_ids && is_array( $chosen_product_ids ) ) {
		...
		if ( $order_items ) {
			...
			foreach ( $order_items as $order_item ) {
				...
				$items_arr = str_replace(array('O:17:"WC_Product_Simple"','O:10:"WC_Product"'), 'O:8:"stdClass"',$order_item->items);
		...
		<h4><?php printf( __( 'Abandoned Carts containing %s:', 'woocommerce' ), implode( ', ', $chosen_product_titles ) ); ?></h4>
		<table class="bar_chart">
			<thead>
				<tr>
					<th><?php _e('Month', 'woocommerce'); ?></th>
					<th colspan="2"><?php _e('Carts', 'woocommerce'); ?></th>
				</tr>
			</thead>
				<tbody>
				<?php
					if (sizeof($product_sales)>0) foreach ($product_sales as $date => $sales) :
						...
						$orders_link = admin_url('edit.php?s&post_status=all&post_type=carts&action=-1&s=' . urlencode( implode( ' ', $chosen_product_titles ) ) . '&end_date='.date('Y-m-d', strtotime(date('Y') . '-' .(date('m') + 1) . '-'.'01') - (60*60*24)).'&start_date=' . date('Y-m-d', strtotime($date.'01')) . '&mv=Abandoned');
					...
					endforeach; else echo '<tr><td colspan="3">'.__('No Carts :)', 'woocommerce').'</td></tr>';
				?>
			</tbody>
		</table>
		<?php
 ...
 */
 function woocommerce_carts_overview() {
	...
	if (!$start_date) $start_date = $current_month;
	if (!$end_date) $end_date = strtotime(date('Ymd', current_time('timestamp')));
	...
	$params = array(
		...
		'number_of_updated_carts'    => __( 'Open and Abandoned Carts', 'woocommerce' ),
	);
...
<div id="poststuff" class="woocommerce-reports-wrap">
		<div class="woocommerce-reports-sidebar">
			<div class="postbox">
<?php 					$link = add_query_arg( array( 'post_type' => 'carts', 'mv' => 'OandA','start_date' => date('Y-m-d', time() - (60*60*24 * $days)), 'end_date' => date('Y-m-d', time()) ), get_admin_url( null, 'edit.php' ) );
						$num_updated  = '<a style = "color:#333;" href="' . $link . '">' . $total_updated  . '</a>';
?>
				<h3><span><?php _e('Open & Abandoned Carts In Range', 'woocommerce'); ?></span></h3>
				<div class="inside">
					<p class="stat"><?php if($total_updated != '') echo $num_updated . ' Carts';  else _e('n/a', 'woocommerce'); ?></p>
				</div>
			</div>
			<div class="postbox">
<?php 					$link = add_query_arg( array( 'post_type' => 'carts', 'mv' => 'Converted','start_date' => date('Y-m-d', time() - (60*60*24 * $days)), 'end_date' => date('Y-m-d', time()) ), get_admin_url( null, 'edit.php' ) );
						$num_converted  = '<a style = "color:#333;"href="' . $link . '">' . $total_converted  . '</a>';
?>
				<h3><span><?php _e('Converted Carts In Range', 'woocommerce'); ?></span></h3>
				<div class="inside">
					<p class="stat"><?php sizeof($total_converted);  if($total_converted != '') echo $num_converted . ' Carts' ; else _e('n/a', 'woocommerce'); ?></p>
				</div>
			</div>
	...
	?>
		...
		jQuery(function(){
			...
			var plot = jQuery.plot(placeholder, [ { label: "<?php echo esc_js( __( 'Converted Carts', 'woocommerce' ) ) ?>", data: d }, { label: "<?php echo esc_js( __( 'Open & Abandoned Carts', 'woocommerce' ) ) ?>", data: d2, yaxis: 1 } ], {
				 series: {
            	stack:true,
                lines: {
                	fill: true,
...
function carts_abandoned_within_range( $where = '' ) {
	...
	if($end_date == strtotime($current_date)){ //echo "hello"; 
		$end_date = time() + ($offset * 3600);
		$before = date('Y-m-d G:i:s', $end_date - $timeout);
	}
	else $before = date('Y-m-d', strtotime("+1 day", $end_date));
	$timeout = $woocommerce_cart_reports_options['timeout'];

...
public function tooltip_scripts() {
	...
	$woocommerce->add_inline_js( "jQuery('.help_tip').tipTip({
    	'attribute' : 'data-tip',
    	'fadeIn' : 50,
    	'fadeOut' : 50,
    	'delay' : 200
...
public function cart_status_customer_metabox(){
	...
	elseif($this->receipt->status() == "Converted")
	{
		...
		echo __( '<p>' . ucwords($order->billing_first_name . ' ' . $order->billing_last_name) . ' (Guest)</p>');	
	}
	else
		echo __( '<span style="color:gray";>Name Not Available</span>' ) . av8_tooltip(__('No customer name information available for carts created by non-logged-in Guests.', 'woocommerce_cart_reports'), false);
...
public function cart_status_metabox(){
    ...
    echo __( '<div id="edit_status"><mark class="color-wrapper '.strtolower($show_custom_state).'_edit">'.$show_custom_state. $tooltip.'</mark></div>' );
}
...
<?php
	...
	$updated_stamp = $this->receipt->last_updated();
	error_log("LAST UPDATED" . $this->receipt->last_updated());
	assert($updated_stamp >0);

	echo date('F j, Y h:i A', $updated_stamp);
...
public function woocommerce_cart_items_meta_box($post) {
	...
	<?php if (sizeof($order_items)>0): ?>
		<table cellpadding="0" width="100%" cellspacing="0" class="woocommerce_cart_reports_items">
			<thead >
				<tr>
					<th class="thumb"  width="20%" style="text-align:left;"><?php _e('Item', 'woocommerce'); ?></th>

					<th class="sku"   width="20%" style="text-align:left"><?php _e('SKU', 'woocommerce'); ?></th>

					<th class="name"  width="20%" style="text-align:left"><?php _e('Name', 'woocommerce'); ?></th>

					<th class="price"  width="20%" style="text-align:left"><?php _e('Price', 'woocommerce'); ?></th>

					<?php //do_action('woocommerce_admin_order_item_headers'); ?>
					
					<th class="quantity"  width="20%" style="text-align:left"><?php _e('Qty', 'woocommerce'); ?></th>
					...
					<tr class="item <?php echo $table_color; ?>" rel="<?php echo $loop; ?>">
						<td class="thumb" width="20%">
							<a href="<?php echo esc_url( admin_url('post.php?post='. $_product->id .'&action=edit') ); ?>" class="tips" data-tip="<?php
								...
								echo '<br/><strong>'.__('Variation ID:', 'woocommerce').'</strong> '; if (isset($item->variation_id)) echo $item->variation_id; else echo '-';
								echo '<br/><strong>'.__('Product SKU:', 'woocommerce').'</strong> '; if (isset($_product->sku)) echo $_product->sku; else echo '-';
						...
						<td class="sku" width="20%">
							...
							<input type="hidden" name="item_name[<?php echo $loop; ?>]" value="<?php echo esc_attr( $item->id ); ?>" />

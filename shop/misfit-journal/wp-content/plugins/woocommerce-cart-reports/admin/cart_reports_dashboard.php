...
public function setup_widgets() {
	...
	//$tooltip = av8_tooltip(__("This graph displays the number of Converted vs Open + Abandoned Carts on a daily display."), false);
	$tooltip = '';
...
public function woocommerce_cart_widget_css() {
	...
	if(is_admin()){
		if(($pagenow == 'index.php' )){
   			...
   			wp_enqueue_style( 'woocommerce_cart_report_admin_dashboard_css', plugins_url() . '/woocommerce-cart-reports/assets/css/cart_reports_admin_dashboard.css');
		}
	}
}
 
/**
 * Widget to display at-a-glance information about cart usage on the site.
 * 
 */
...
<div class="table table_carts_right_now">
		<p class="sub woocommerce_sub"><?php _e( 'Right Now', 'woocommerce' ); ?></p>
		<table>
			<tr class="first">

				<?php
					$num  = number_format_i18n( $nums_lifetime['Open'] );
					$text = __( 'Open', 'woocommerce' );
					$link = add_query_arg( array( 'post_type' => 'carts', 'mv' => 'Open' ), get_admin_url( null, 'edit.php' ) );
	...
	<div class="table table_carts">
		<p class="sub woocommerce_sub"><?php _e( "The Last $days Day" . $plural, 'woocommerce' ); ?></p>
		<table>
			<tr class="first">

				<?php
					$num  = number_format_i18n( $nums_right_now['Abandoned']  ); //We're adding these together for the "Not Converted" field.
					$text = __( 'Abandoned', 'woocommerce' );
					$link = add_query_arg( array( 'post_type' => 'carts', 'mv' => 'Abandoned','start_date' => date('Y-m-d', time() - (60*60*24 * $days)), 'end_date' => date('Y-m-d', time()) ), get_admin_url( null, 'edit.php' ) );
					$num  = '<a href="' . $link . '">' . $num  . '</a>';
					$text = '<a href="' . $link . '">' . $text . '</a>';
				?>
	...
	<div class="table table_carts_life">
		<p class="sub woocommerce_sub"><?php _e( 'Lifetime', 'woocommerce' ); ?></p>
		<table>
			<tr class="first">

				<?php
					$num  = number_format_i18n( $nums_lifetime['Abandoned'] ); //We add open + abandoned to get "Not Converted"
					$text = __( 'Abandonded', 'woocommerce' );
					$link = add_query_arg( array( 'post_type' => 'carts', 'mv' => 'Abandoned','lifetime'=>'' ), get_admin_url( null, 'edit.php' ) );
					$num  = '<a href="' . $link . '">' . $num  . '</a>';
					$text = '<a href="' . $link . '">' . $text . '</a>';
				?>
			...
			<tr>
				...
				<?php
					...
					$text = __( 'Converted', 'woocommerce' );
					$link = add_query_arg( array( 'post_type' => 'carts', 'mv' => 'Converted','lifetime'=>'' ), get_admin_url( null, 'edit.php' ) );
					$num  = '<a href="' . $link . '">' . $num  . '</a>';
					$text = '<a href="' . $link . '">' . $text . '</a>';
				?>
...
public function woocommerce_dashboard_carts_js() {
	...
	$params = array(
		...
		'number_of_updated_carts'    => __( 'Open + Abandoned Carts', 'woocommerce' ),
	...
	wp_register_script( 'woocommerce-dashboard-carts', get_bloginfo('url') . '/wp-content/plugins/woocommerce-cart-reports/assets/js/dashboard_carts'.$suffix.'.js', 'jquery', '1.0' );

	wp_register_script( 'carts-flot-carts-resize', get_bloginfo('url') . '/wp-content/plugins/woocommerce-cart-reports/assets/js/jquery.flot.resize.js', 'jquery', '1.0' );
	wp_enqueue_script( 'flot-carts', get_bloginfo('url') . '/wp-content/plugins/woocommerce-cart-reports/assets/js/jquery.flot.min.js' );
	...
	wp_enqueue_script( 'flot-carts-resize', get_bloginfo('url') . '/wp-content/plugins/woocommerce-cart-reports/assets/js/jquery.flot.resize.min.js' );

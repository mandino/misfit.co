...
public function enqueue_index() {
	...
	if($pagenow == 'edit.php' && isset($_GET['post_type']) && $_GET['post_type'] == 'carts' ){
	    ...
	    //wp_register_script( 'ajax-chosenav8', $woocommerce->plugin_url() . '/assets/js/chosen/ajax-chosen.jquery.js', array('jquery', 'chosen'), $woocommerce->version );
		//wp_register_script( 'chosenav8', $woocommerce->plugin_url() . '/assets/js/chosen/chosen.jquery.js', array('jquery'), $woocommerce->version );

    	//wp_enqueue_script('ajax-chosenav8');
    	//wp_enqueue_script('chosenav8');  
    	/*
    	$woocommerce->add_inline_js( "
    
    		jQuery('select[name=mv]').css('width', '150px').chosen();

						jQuery('select#author').css('width','250px').ajaxChosen({
						    method: 		'GET',
...
public function restrict_manage_posts() {
    ...
    if(($pagenow == 'edit.php') && isset($_GET['post_type']) && ($_GET['post_type'] == 'carts'))
    {
		$status_options = array("Open" =>"Open", "Converted" =>"Converted", "Abandoned" =>"Abandoned", "Open + Abandoned"=>"OandA");
		$authors = get_users();
		...
		?>
		<label for="from"><?php _e('From:', 'woocommerce'); ?></label> <input type="text" name="start_date" id="from" readonly="readonly" value="<?php echo esc_attr( date('Y-m-d', $start_date) ); ?>" /> <label for="to"><?php _e('To:', 'woocommerce'); ?></label> <input type="text" name="end_date" id="to" readonly="readonly" value="<?php echo esc_attr( date('Y-m-d', $end_date) ); ?>" /> 
	...
	if ( !empty($users) && ( empty($hide_if_only_one_author) || count($users) > 1 ) ) {
		...
		foreach ( (array) $users as $user ) {
    		...
    		if($usermeta['first_name'][0] != '' && $usermeta['last_name'][0])
    			$full_name = ucwords($usermeta['first_name'][0]) . ' '. ucwords($usermeta['last_name'][0]);
    		elseif((isset($usermeta['billingfirst_name']) && $usermeta['billingfirst_name'][0]) != '' && (isset($usermeta['billinglast_name']) && $usermeta['billinglast_name'][0] != ''))
    			$full_name = ucwords($usermeta['billingfirst_name'][0]) . ' '. ucwords($usermeta['billinglast_name'][0]);
			else
...
if($pagenow == 'edit.php' && isset($_GET['post_type']) && $_GET['post_type'] == 'carts'){
	...
	if(isset($_GET['mv'])) {
		if($_GET['mv'] == "Open")
			$where .= " AND post_date > '".date('Y-m-d G:i:s', gmmktime() + ($offset * 3600) - $timeout)."'";

...
class AV8_Cart_Receipt {
	...
	public function set_products($woocommerce) {
		...
		$this->products = $this->generic_objects($products);
	}
	...
	*/
	...
	assert($this->post_author > -1); //Is 0 or greater
	if($this->post_author == 0)
		return true;
	else
		return false;
	...
	public function load_receipt($post_id) {
		...
		$this->last_updated = get_post_meta($this->post_id, "av8_last_updated",true) + ($offset * 3600);
	...
	public function very_recent($where) {
		...
		$where .= " AND post_date > '" . date('Y-m-d G:i:s',  gmmktime() +($offset*3600)  - 8 ). "' ";
		return $where;
	}
	...
	public function status() {
		...
		$since_connected = gmmktime() + ($offset*3600) - $this->last_connected();
		if( $since_connected > $timeout && $this->last_connected() != '' && $stored_status != "Converted" && $timeout > 0)
			return ABANDONED;
...
public function email() {
	...
	public function last_connected() {
	...
	public function set_last_connected() {
			update_post_meta($this->post_id, "av8_last_connected", gmmktime());
	...
	public function get_age_text($time = '') {
		...
		else
		{
			...
			if($number_days > 180) return __("> 6 Months","woocommerce_cart_reports");
	...
	public function save_order_id($order_id, $woocommerce) {
		assert($order_id > 0 && is_int($order_id) && $order_id != '');
		update_post_meta($this->post_id, "av8_order_id", $order_id);
		$order = $woocommerce->checkout->posted; //Get new data
		$post_title =  $order['billing_first_name'] . ' ' . $order['billing_last_name'];
		$post_updated = array(
		   'ID' => $this->post_id,
	...
	public function insert_receipt() {
		...
		$post = array(
			'post_author'  => $this->post_author, //The user ID number of the person who created this cart. If it's guest, this value will be emtpy. Wordpress doesn't complain, so at this point neither do I.
			'post_content' => '',
			'post_status'  => 'publish',
			'post_title'   => $this->post_title,
			'post_type'    => 'carts'
		...
		update_post_meta( $this->post_id, "cart_session_id", $this->sid ); //Save the session id for this cart.

		if ( isset( $woocommerce_cart_reports_options['logip'] ) ) { //Only save if the user has checked "Log IP" in the setting page
			update_post_meta( $this->post_id, "av8_ip_address", $this->ip_address );
			update_post_meta( $this->post_id, "av8_user_agent", $this->user_agent );
		}

		wp_set_object_terms( $this->post_id, 'open', "shop_cart_status" ); //Saving the open/converted as meta - much faster than postmeta!

		update_post_meta( $this->post_id, 'av8_cartitems', $this->add_titles_to_cart_items( $this->products ) ); //Save the Cart products from the woocommerce object

		update_post_meta( $this->post_id, 'av8_cart_created', gmmktime() ); //Save created time
		update_post_meta( $this->post_id, 'av8_last_updated', gmmktime() ); //Initialize the updated time
	}
	...
	public function print_cart_actions() {
		...
		case 'Open':
			...
			//if(($post->post_author != '' && $post->post_author > 0 || $cart_receipt->email() != '')) {
			//	assert($cart_receipt->email() != '');

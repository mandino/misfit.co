...
class Product_Addon_Display {
	...
	public function __construct() {
		...
		// Styles
		add_action( 'wp', array( $this, 'styles' ) );
		...
		add_filter( 'add_to_cart_text', array( $this, 'add_to_cart_text'), 10 );
	 ...
	 * @return void
	 */
	function display( $post_id = false ) {
		global $product;
		...
		$params = array(
			'i18n_addon_total' => __( 'Add-ons total:', 'wc_product_addons' ),
	...
	function totals( $post_id ) {
		...
		echo '<div id="product-addons-total" data-type="' . $product->product_type . '" data-price="' . ( is_object( $product ) ? $product->get_price() : '' ) . '"></div>';
	}
